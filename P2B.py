...
# Function to generate status message for hypothesis test
def status_res(val1, val2, reason):
    if len(reason) != 2:
        raise ValueError("Reason should contain exactly two elements.")
    
    return (
        f"Reject the null hypothesis: {reason[0]}" if val1 > val2
...
def perform_anova(groups, alpha=0.05):
    ...
    dfd = sum(len(group) for group in groups) - len(groups)  # Degrees of freedom for denominator
    f_critical = stats.f.ppf(1 - alpha, dfn, dfd)
    status = status_res(anova_result.statistic, f_critical, 
                        ["Significant differences exist among groups", "No significant differences among groups"])
...
def perform_chi2_test(contingency_table, alpha=0.05):
    ...
    status = status_res(chi2_stat, chi2_critical, 
...
titanic_data = titanic_data.dropna(subset=["Age", "Pclass_1", "Survived"])

# Perform ANOVA: Impact of passenger class on age
grouped_ages = [
    titanic_data[titanic_data["Pclass_1"] == 1]["Age"],
    titanic_data[titanic_data["Pclass_2"] == 2]["Age"],
    titanic_data[titanic_data["Pclass_3"] == 3]["Age"],
]
perform_anova(grouped_ages)

# Perform Chi-Square Test: Relationship between survival status and passenger class
contingency_table = pd.crosstab(titanic_data["Pclass_1"], titanic_data["Survived"])
